...
```{r visualize-network, fig.width=8, fig.height=6}
...
```{r visualize-results, fig.width=8, fig.height=6}
...
The `africa_segments` dataset provides raw network segments that can be processed through the full consolidation workflow:
...
The `consolidate_graph()` function removes intermediate nodes (nodes that occur exactly twice) and merges connecting edges:
...
Key arguments:
- `keep`: Node IDs to preserve (city/port nodes)
- `w`: Weight column for aggregation (passes = number of routes using each segment)

### Compare Original vs Consolidated Network

```{r compare-networks, fig.width=8, fig.height=6}

...
This formulation ensures that the model assigns lower probabilities to routes that are largely duplicates of others, distributing flow more realistically across truly distinct alternatives.
...
**`simplify_network()`**: Simplifies the network by keeping only edges traversed by shortest paths between specified nodes. Useful for reducing network size when only certain OD pairs are of interest.
...
- **`beta`**: The path-sized logit parameter. Typically positive (default 1) to penalize route overlap.
